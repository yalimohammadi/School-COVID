...
def fast_SIR(G, tau, gamma, initial_infecteds=None, initial_recovereds=None,
             ...
             all_test_times=[], fraction_of_infections_from_community_per_day=0.0, test_args=None,test_func=None,weighted_test=True,school=None, isolate=False, com_inf_dict=[], test_fraction=0, trans_array=[]):
    r'''
    ...
    **full_data**  Simulation_Investigation object
    ...
    if transmission_weight is not None or tau * gamma == 0:
        ...
        def trans_time_fxn(source, target, trans_rate_fxn, trans_array):
            ...
            if rate > 0:
                if trans_array[source][target]>0:
                    return trans_array[source][target]
                else:
        ...
        return fast_nonMarkov_SIR(G, trans_time_fxn=trans_time_fxn,
                                  ...
                                  school=school,isolate=isolate, com_inf_dict=com_inf_dict, test_fraction=test_fraction,trans_array=trans_array)
        ...
        return fast_nonMarkov_SIR(G,
                                  ...
                                  rho=rho, tmin=tmin, tmax=tmax,
                                  return_full_data=return_full_data,
                                  sim_kwargs=sim_kwargs,com_inf_dict=com_inf_dict, test_fraction=test_fraction,trans_array=trans_array)
...
def fast_nonMarkov_SIR(G, trans_time_fxn=None,
                       ...
                       weighted_test=True, school=None,isolate=False,com_inf_dict=[],test_fraction=0,trans_array=[]):
    r'''
    ...
    **full_data**  Simulation_Investigation object
    ...
    times, S, E, I,NI, P, R, Isolated, CI = ([tmin], [G.order()],[0], [0], [0], [0], [0],[0],[0]) #CI = community infected
    transmissions = []
    ...
    while Q or cur_test_time< tmax: #or community_spread_time < tmax   # all the work is done in this while loop.
        if Q:
        ...
        #             # pred_inf_time[u] = tmin + inf_time
        #             Q.add(community_spread_time + inf_time, _process_trans_SIR_, args=(G, -1, u, times, S, E, I,NI, P, R, Isolated, CI,Q,
        #                                                           status, at_school, rec_time,
        #                                                           pred_inf_time, transmissions,
        #                                                           trans_and_rec_time_fxn,
        #                                                           trans_and_rec_time_args
    ...
    # COMMUNITY SPREAD CODE ENDS HERE
        ...
        if cur_test_time < cur_time: # and cur_test_time < community_spread_time:
            if weighted_test:
                ...
                positive_nodes = testing_strategy(cur_test_time, times, S, E, I,NI, P, R, Isolated, CI, status,tested,test_args,test_func,at_school,test_fraction) # call process_test_SIR on all indivduals who are in state S and I and they are tested at that particular moment
            if isolate:
                new_isolated=find_isolated_cohorts(positive_nodes,school,at_school)
                isolate_set_of_nodes(cur_test_time,times,S,E,I,NI,P,R,Isolated,CI,Q,new_isolated, at_school)
            else:
                isolate_set_of_nodes(cur_test_time,times,S,E,I,NI,P,R,Isolated,CI,Q,positive_nodes,at_school)
            if len(all_test_times)>0:
                cur_test_time = all_test_times.pop(0)
            else:
                cur_test_time = tmax + 10
    ...
    CI = CI[len(initial_infecteds):]
    Isolated = Isolated[len(initial_infecteds):]
    if not return_full_data:
        return np.array(times), np.array(S), np.array(E), np.array(I), np.array(NI), \
               np.array(P), np.array(R), np.array(Isolated),np.array(CI), status,transmision_from_community
    else:
...
def isolate_set_of_nodes(time, times, S, E, I, NI, T, R, Isolated, CI,Q, set_of_nodes,at_school,isolation_time=14.0):
    for node in set_of_nodes:
        _isolate_a_node(time, times, S, E, I, NI, T, R, Isolated, CI,Q, node, at_school,isolation_time=isolation_time)

def _isolate_a_node(time, times, S, E, I, NI, P, R, Isolated, CI,Q, node, at_school, isolation_time=14.):
...
def _unisolate_a_node(time, times, S, E, I, NI, P, R, Isolated,CI, node, at_school):
...
def testing_strategy(time, times, S, E, I, NI, P, R, Isolated, CI,status, tested, test_args, test_func,at_school,test_fraction=0):
    if test_fraction==1:
        to_test = tested.keys()
    else:
...
def testing_strategy_with_weights(time, times, S, E, I, NI, P, R, Isolated, CI, status, tested, school, test_cap, curr_weight, next_weight):
    ...
    if status[target] == 'E': #and status[source] == 'I':  # nothing happens if already infected/tested.
        ...
        if rec_time[target] <= Q.tmax:
            Q.add(rec_time[target], _process_rec_SIR_,
                  args=(target, times, S, E, I, NI, P, R, Isolated,CI, status))
...
def _process_exp_SIR_(time, G, source, target, times, S, E, I, NI, P, R, Isolated, CI, Q, status, at_school,
    ...
    '''
    if status[source] == 'I' and status[target] == 'S' and (from_comm or (at_school[source] and at_school[target])):
        times.append(time)
        S.append(S[-1]-1)  # no change to number susceptible
        I.append(I[-1])  # one less infected
...
def _process_rec_SIR_(time, node, times, S, E, I, NI, P, R, Isolated, CI, status):
    r'''From figure A.3 of Kiss, Miller, & Simon.  Please cite the
    book if using this algorithm.
    :Arguments:
        event : event
