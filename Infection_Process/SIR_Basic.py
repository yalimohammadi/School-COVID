...
def _process_trans_SIS_nonMarkov_(time, G, source, target, future_transmissions,
    ...
    **target** node
            ...
            if trans_delays[v]:
                ...
                if trans_times:  # no point adding any if there are none
                    Q.add(trans_times[0], _process_trans_SIS_nonMarkov_,
                          args=(G, target, v, following_transmissions, times, S, I, Q, status,
    ...
    if trans_times:
        Q.add(trans_times[0], _process_trans_SIS_nonMarkov_,
              args=(G, source, target, following_transmissions, times, S, I, Q, status,
                    rec_time, infection_times, recovery_times, transmissions,
                    trans_and_rec_time_fxn, trans_and_rec_time_args))
...
def Gillespie_SIR(G, tau, gamma, initial_infecteds=None,
    ...
    **full_data**  Simulation_Investigation object
    ...
    if not return_full_data:
        ...
        if sim_kwargs is None:
            sim_kwargs = {}
        return EoN.Simulation_Investigation(G, node_history, transmissions, possible_statuses=['S', 'I', 'R'],
                                            **sim_kwargs)
...
'''
    ...
    if not return_full_data:
        ...
        if sim_kwargs is None:
            sim_kwargs = {}
        return EoN.Simulation_Investigation(G, node_history, transmissions, possible_statuses=return_statuses,
                                            **sim_kwargs)
