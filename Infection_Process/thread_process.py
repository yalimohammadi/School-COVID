...
def function_run_one_sim(param_list):
    all_test_times = param_list[0]
    G=param_list[1]
    school =param_list[2]
    number_of_tests=param_list[3]
    test_fraction = param_list[4]
    fraction_infected_at_each_time_step_from_community = param_list[5]
    removal_rate = param_list[6]
    transmission_scale = param_list[7]
    initial_fraction_infected =param_list[8]
    tmax=param_list[9]
    ...
    t, S, E, I, NI, T, R, Isolated, CI, status, total_infections_from_community = EoN.fast_SIR(G, gamma=removal_rate,
                                                                                               ...
                                                                                               isolate=True,
                                                                                               tmax=tmax,
                                                                                               com_inf_dict=com_inf_dict,
                                                                                               test_fraction=test_fraction,
                                                                                               trans_array=trans_array)
    return R[-1] + I[-1] #test if Pool works
...
def SIR_on_weighted_Graph(all_test_times, G, school, number_of_tests=0,test_fraction=0, fraction_infected_at_each_time_step_from_community=0,
    ...
    param_list_one_sim=[all_test_times, G, school, number_of_tests,test_fraction, fraction_infected_at_each_time_step_from_community,
                          removal_rate, transmission_scale, initial_fraction_infected,tmax]

    pass_arg=[param_list_one_sim]*num_sim
    all_final_infecteds=[]
    with Pool(10) as p:
        all_final_infecteds = p.map(function_run_one_sim, pass_arg)
